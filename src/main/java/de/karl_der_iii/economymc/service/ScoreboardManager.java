    ...
    public static void update(MinecraftServer server) {
        try {
            ...
            for (Map.Entry<UUID, Long> entry : entries) {
                ...
                server.getCommands().performPrefixedCommand(
                    source,
                    "scoreboard players set " + scoreboardName(playerName) + " " + OBJECTIVE + " " + entry.getValue()
                );
            }
            ...
            server.getCommands().performPrefixedCommand(
                source,
                "scoreboard players set " + scoreboardName(treasuryName) + " " + OBJECTIVE + " " + TreasuryManager.getTreasury()
            );
